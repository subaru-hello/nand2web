...
export const INSTRUCTIONS: readonly InstructionSpec[] = [
  ...
  { mnemonic: "LDI", opcode: 0x1, operand: "immediate", description: "A ← n (load immediate)." },
  { mnemonic: "TAB", opcode: 0x2, fixedOperand: 0x0, operand: "none", description: "B ← A (transfer A to B)." },
  { mnemonic: "TBA", opcode: 0x2, fixedOperand: 0x1, operand: "none", description: "A ← B (transfer B to A)." },
  { mnemonic: "ADD", opcode: 0x3, fixedOperand: 0x0, operand: "none", description: "A ← A + B; sets carry and zero." },
  { mnemonic: "SUB", opcode: 0x3, fixedOperand: 0x1, operand: "none", description: "A ← A − B; carry = no-borrow; sets zero." },
  { mnemonic: "AND", opcode: 0x3, fixedOperand: 0x2, operand: "none", description: "A ← A AND B; sets zero." },
  { mnemonic: "OR", opcode: 0x3, fixedOperand: 0x3, operand: "none", description: "A ← A OR B; sets zero." },
  { mnemonic: "LD", opcode: 0x4, operand: "address", description: "A ← RAM[n]." },
  { mnemonic: "ST", opcode: 0x5, operand: "address", description: "RAM[n] ← A." },
  { mnemonic: "JMP", opcode: 0x6, operand: "label", description: "PC ← n (unconditional jump)." },
  { mnemonic: "JZ", opcode: 0x7, operand: "label", description: "PC ← n if the zero flag is set." },
  { mnemonic: "JC", opcode: 0x8, operand: "label", description: "PC ← n if the carry flag is set." },
  { mnemonic: "OUT", opcode: 0x9, fixedOperand: 0x0, operand: "none", description: "OUT ← A (display A)." },
  { mnemonic: "IN", opcode: 0x9, fixedOperand: 0x1, operand: "none", description: "A ← IN (read the input port)." },
  { mnemonic: "HLT", opcode: 0xf, operand: "none", description: "Halt the machine." },
];
...
export function encode(spec: InstructionSpec, operand: number): number {
  return ((spec.opcode & WORD_MASK) << 4) | ((spec.fixedOperand ?? operand) & WORD_MASK);
}
